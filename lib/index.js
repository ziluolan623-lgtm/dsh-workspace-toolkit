...
//      —— 需要 workspaceRegistry + sessionPersistence + subprocess + webServer + agents
//      五个服务；五者用同一层 ctx.inject 一起等待，任一缺失整块归档功能不激活，
...
export function apply(ctx) {
  ...
  // 功能 2-6：归档面板的完整后端。agents 用来阻止删除仍在运行的会话。
  ctx.inject(['workspaceRegistry', 'sessionPersistence', 'subprocess', 'webServer', 'agents'], (services) => {
    services.effect(() => {
